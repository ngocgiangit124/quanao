...
class ImageLib {
  ...
  public static function saveFileGif($file, $dirictory, $fileName, $resizes = array(), $crop = 0, $type='.jpg', $background = '') {
    if (is_dir($dirictory) != true){
      $permission = intval('0777', 8);
      File::makeDirectory($dirictory, $permission, true);
    }
    $dirictory = $dirictory.'/';
    file_put_contents($dirictory.$fileName.$type, fopen($file, "r"));
    if (!empty($resizes)) {
      foreach ($resizes as $resize) {
        file_put_contents($dirictory.$fileName.'_'.$resize.$type, fopen($file, "r"));
        File::move($dirictory.$fileName.'_'.$resize.$type, $dirictory.$fileName.'_'.$resize.'.png');
      }
    }
    File::move($dirictory.$fileName.$type, $dirictory.$fileName.'.png');
  ...
  public static function upload_image_getcontent($file, $dirictory, $fileName, $resizes = array(), $crop = 0, $type='.jpg', $background = '') {
        if (is_dir($dirictory) != true){
          $permission = intval('0777', 8);
          File::makeDirectory($dirictory, $permission, true);
        }
        ...
        // if($mime == 'image/gif') {
        //   ImageLib::saveFileGif($file, $dirictory, $fileName, $resizes = array(), $crop = 0, $type='.jpg', $background = '');
        // } else {
          $imgsource->save($dirictory.$fileName.$type);
          if (!empty($resizes)) {
            foreach ($resizes as $resize) {
  ...
  public static function delete_image2($dirictory, $fileName, $resizes = array(), $type='.png') {
        // echo 'a';
        //echo $dirictory.'/'.$fileName.$type;die;
//        File::delete($dirictory.'/'.$fileName.$type);
      if (!empty($resizes)) {
  ...
  public static function rename_image($dirictory, $file_name, $resizes) {
    if (!empty($resizes)) {
      foreach ($resizes as $resize) {
        $size = explode("x",$resize);
        $width = $size[0];
        $height = $size[1];
        $image_old = $dirictory.'/'.$file_name.'_'.$resize.'.jpg';
        $image_new = $dirictory.'/'.$file_name.'_'.$resize.'.png';
